...
When the level selector is set to maximum intensity (100%), the interface
...
## The Level Selector and Color Rays

The level selector controls the intensity of the corner view via scalar
multiplication. As the level selector is adjusted, a scaling factor is applied
to every color visible on the faces. Geometrically, this visualizes points
traveling along an **intensity ray**: a straight line originating from black at
the hidden far corner of the 3D cube model and terminating at a coordinate on
one of the outer faces.
...
Because the level selector acts as a direct multiplier, setting it to 50% cuts
all three of these channel values exactly in half. Red remains at minimum, Green
drops to a quarter-capacity, and Blue drops to half-capacity. This proportional
...
center point while lowering the level selector pushes the color straight down
...
- **Value / Brightness (The Height):** The depth along the intensity ray, which
  is mapped directly to the level selector.

By separating the spectrum into these three dimensions, the geometry of the tool
aligns with human perception. Instead of forcing users to guess Cartesian
coordinates, it isolates the *identity* of a color (Hue) from its *purity*
